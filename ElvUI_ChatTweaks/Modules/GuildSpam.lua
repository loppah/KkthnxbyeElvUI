...
local players = {}
local notifyString = L["%s from |cff00ff00%s|r was blocked."]
...
local function ChannelFilter(_, event, message, player, _, _, _, _, channel, _, _, _, id)
	...
	if chanid == 0 or chanid == 25 then result = nil; return end
	if not CanComplainChat(id) or UnitIsInMyGuild(player) then result = nil; return end
	for i = 1, #Module.triggers do
		if message:lower():find(Module.triggers[i]) then
			if db.notify then
				Module:Print(notifyString:format(L["Chat message"], player)) 
			end
...
local function WhisperFilter(_, event, message, player)
	...
	if not db.whispers or players[player] or not CanComplainChat(player) or
		UnitIsInMyGuild(player) or UnitInRaid(player) or UnitInParty(player) then return end
