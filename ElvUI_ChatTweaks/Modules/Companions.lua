...
function Module:SendZoneList()
	...
	for i = 1, C_PetJournal.GetNumPets(false) do
		...
		if string.find(tooltip, zone) and not found[name] then
			...
			zone = {
				...
				args = {
					about = {
						...
						order = 1,
						width = "full",
						name = L["Enabling this will send a message to your chat whenever you change a zone with a list of the Battle Pets that are obtainable in the zone.  You have the option to filter them based on whether or not you own them.  This will also add the chat commands |cff00ff96/zonelist|r and |cff00ff96/zl|r to send the list whenever.  Even if you disable this the chat commands will still work."],
					},
					enable = {
						type = "toggle",
						order = 2,
...
function Module:Info()
	return L["Adds the chat commands |cff00ff96/pets|r and |cff00ff96/mounts|r to provide information about your mounts and pets."]
end