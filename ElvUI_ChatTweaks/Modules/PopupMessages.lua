...
local function FindKeywords(self, event, text, author, ...)
	...
	if found then
		...
		-- play a sound if its enabled
		if db.playSound and LSM:IsValid("sound", db.sound) then
			PlaySoundFile(LSM:Fetch("sound", db.sound), "Master")
		end
...
function Module:RefreshChannels()
	for index, value in pairs(db.channels) do
		if value == true then
			ChatFrame_AddMessageEventFilter(("CHAT_MSG_%s"):format(value:upper()), FindKeywords)
		else
			ChatFrame_RemoveMessageEventFilter(("CHAT_MSG_%s"):format(value:upper()), FindKeywords)
		end
	end
end
	...
	if not options then
		options = {
			...
			sound = {
				...
				disabled = function() return not db.playSound end,
