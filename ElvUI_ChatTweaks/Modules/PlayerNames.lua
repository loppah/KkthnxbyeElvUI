...
local function changeName(header, name, extra, count, display, body)
	...
	if level and (level ~= MAX_PLAYER_LEVEL or not Module.db.profile.excludeMaxLevel) then
		if Module.db.profile.levelByDiff then
			local color = GetQuestDifficultyColor(level)
			level = ("|cff%02x%02x%02x%s|r"):format(color.r * 255, color.g * 255, color.b * 255, level)
		else
			if display:match("|cff......") then
				level = display:gsub(display, "((|cff......).-|r)", function(string, color)
			...
			level = {
				...
				args = {
					...
					colorLevelByDifficulty = {
						...
						order = 3,
						disabled = function() return not Module.db.profile.includeLevel end,
						get = function() return Module.db.profile.levelByDiff end,
						set = function(_,  v)
							Module.db.profile.levelByDiff = v
							Module:WipeCache()
						end,
					},
					excludeMaxLevel = {
